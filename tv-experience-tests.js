...
check(!/iptv|xtream|m3u/i.test(index + mobile + lite + account), 'legacy private TV catalogue code is still exposed');
...
check(tvPwa.includes('beforeinstallprompt') && tvPwa.includes("serviceWorker.register('./tv-sw.js?v=4'") && tvPwa.includes('Installa app') && tvServiceWorker.includes("'/api/'") && tvServiceWorker.includes('fetch(request)'), 'Android TV PWA install or network-first shell is incomplete');
...
check(index.includes("rewind24:{name:'Rewind 24h'") && index.includes("rewind3:{name:'Rewind 3'") && mobile.includes("rewind24: {name:'Rewind 24h'") && mobile.includes("rewind3: {name:'Rewind 3'") && liveStateApi.includes("'rewind24', 'rewind3', 'rewind7', 'rewind30'"), 'Rewind channels are not available on desktop, mobile and API');
check(index.includes('function skipRewindVideo()') && mobile.includes('function skipMobileRewindVideo()') && liveStateApi.includes("'personalAdvanceApplied'"), 'personal Rewind skip is incomplete');

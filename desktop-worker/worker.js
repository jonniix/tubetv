...
function touchViewer(job,viewer){job.lastAccess=Date.now();if(viewer)job.viewers.set(String(viewer).slice(0,64),Date.now());for(const[id,time]of job.viewers)if(time<Date.now()-45000)job.viewers.delete(id)}
function waitReady(job,timeout=45000){return new Promise(resolve=>{const until=Date.now()+timeout;const check=()=>{if(fs.existsSync(path.join(job.dir,'master.m3u8'))){job.ready=true;return resolve(true)}if(job.failed||job.stopped||Date.now()>until)return resolve(false);setTimeout(check,250)};check()})}
function launchAdaptive(job){
  ...
  child.stderr.on('data',chunk=>{const value=chunk.toString();job.stderr=(job.stderr+value).slice(-12000);if(!job.ready&&fs.existsSync(path.join(job.dir,'master.m3u8')))job.ready=true});
  ...
  child.on('close',code=>{
    ...
    if(Date.now()-job.lastAccess<90000&&job.restarts<20){job.restarts++;lastFailure='ADAPTIVE_SOURCE_RECONNECTING';setTimeout(()=>launchAdaptive(job),Math.min(12000,2500+job.restarts*750));return}
    job.failed=true;stopStream(job,true);
  });
}
async function startAdaptive(url,viewer){const id=crypto.createHmac('sha256',SECRET).update(url).digest('hex').slice(0,32);let job=streams.get(id);if(job&&!job.failed&&!job.stopped){totalReused++;touchViewer(job,viewer);return job}if(activeJobs>=MAX_JOBS)return null;const dir=path.join(ROOT,id);removeDir(dir);fs.mkdirSync(dir,{recursive:true});job={id,url,dir,createdAt:Date.now(),lastAccess:Date.now(),viewers:new Map(),ready:false,failed:false,stopped:false,stderr:'',restarts:0,generation:0};touchViewer(job,viewer);streams.set(id,job);activeJobs++;totalTranscodes++;launchAdaptive(job);return job}
...
function health(res){queryGpu(gpu=>{let viewers=0,saved=0;const profiles={sd:0,hd:0,fullhd:0};const now=Date.now();for(const job of streams.values()){touchViewer(job,'');viewers+=job.viewers.size;saved+=Math.max(0,job.viewers.size-1);if(job.ready){profiles.sd++;profiles.hd++;profiles.fullhd++}}const recentBytes=byteSamples.reduce((sum,item)=>sum+item[1],0);json(res,200,{ok:true,worker:'TubeTV Desktop Assist',mode:'adaptive-hls',activeJobs,maxJobs:MAX_JOBS,activeStreams:streams.size,viewers,providerConnectionsSaved:saved,profiles,outputMbps:Math.round(recentBytes*8/60000/1000*100)/100,totalBytes,totalTranscodes,totalCompleted,totalFailed,totalReused,lastFailure:lastFailure?'AVAILABLE':'',uptimeSeconds:Math.floor((now-startedAt)/1000),gpu:'NVIDIA NVENC',gpuStats:gpu})})}

setInterval(()=>{const now=Date.now();for(const job of [...streams.values()]){touchViewer(job,'');if(now-job.lastAccess>90000)stopStream(job)}},10000).unref();
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='POST'&&requestUrl.pathname==='/hls/start'){
    let body;try{body=await readBody(req)}catch(_){return json(res,413,{ok:false,error:'BODY_INVALID'})}if(!authorized(req,body))return json(res,401,{ok:false,error:'WORKER_AUTH_INVALID'});let data;try{data=JSON.parse(body)}catch(_){return json(res,422,{ok:false,error:'JSON_INVALID'})}const url=String(data.url||''),viewer=String(data.viewer||'');try{if(!(await allowedSource(url)))return json(res,422,{ok:false,error:'SOURCE_NOT_ALLOWED'})}catch(_){return json(res,422,{ok:false,error:'SOURCE_DNS_FAILED'})}const job=await startAdaptive(url,viewer);if(!job)return json(res,429,{ok:false,error:'WORKER_BUSY'});const ready=await waitReady(job);if(!ready)return json(res,502,{ok:false,error:'WORKER_HLS_START_FAILED'});return json(res,200,{ok:true,id:job.id,ready:true,profiles:['sd','hd','fullhd'],reused:job.createdAt<Date.now()-1000});
  }

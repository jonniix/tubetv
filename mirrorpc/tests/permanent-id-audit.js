...
async function run() {
  ...
  const prepared = await call('Runtime.evaluate', { expression: `(()=>{document.getElementById('openJoin').click();const input=document.getElementById('permanentPortalId');input.value='999999999998';input.dispatchEvent(new Event('input',{bubbles:true}));document.getElementById('joinPermanent').click();return true})()`, returnByValue: true });
  ...
  if (!state.dialog || !/non registrato|offline|non raggiungibile/i.test(state.toast)) throw new Error(`Stato PC non mostrato nel dialog: ${JSON.stringify(state)}`);

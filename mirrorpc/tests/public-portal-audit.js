...
async function run() {
  ...
  await call('Page.navigate', { url: 'http://127.0.0.1:4177/display.html' }); await new Promise(resolve => setTimeout(resolve, 500));
  const viewer = await call('Runtime.evaluate', { expression: `Boolean(document.getElementById('openViewerScanner')&&document.getElementById('accessPassword'))`, returnByValue: true }); if (!viewer.result.result.value) throw new Error('Scanner o password mancanti nella PWA display');
  await call('Runtime.evaluate', { expression: `document.getElementById('openViewerScanner').click()` }); const viewerScanner = await call('Runtime.evaluate', { expression: `!document.getElementById('viewerScannerDialog').classList.contains('hidden')`, returnByValue: true }); if (!viewerScanner.result.result.value) throw new Error('Scanner QR PWA non aperto');
  socket.close(); console.log('Portale pubblico: privacy, mobile, download, password e scanner QR OK');
}
